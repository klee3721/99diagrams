...
export type DemoDiagram = {
  ...
  layers?: (language: Language) => DiagramLayer[]
  snapshot: (language?: Language) => DiagramSnapshot
}

const defaultLayers = (language: Language): DiagramLayer[] => [{ id: 'default', name: text(language, 'Mặc định', 'Default'), visible: true, locked: false }]
const systemLayers = (language: Language): DiagramLayer[] => [
  { id: 'client', name: text(language, 'Máy khách', 'Client'), visible: true, locked: false },
  { id: 'service', name: text(language, 'Dịch vụ', 'Service'), visible: true, locked: false },
  { id: 'data', name: text(language, 'Dữ liệu', 'Data'), visible: true, locked: false },
]
...
function productFlow(language: Language): DiagramSnapshot {
  const nodes = [
    ...
    node('demo-product-ready', 'decision', text(language, 'Sẵn sàng xây dựng?', 'Ready to build?'), 760, 44),
    node('demo-product-build', 'process', text(language, 'Xây bản thử nghiệm', 'Build MVP'), 1010, 60),
    node('demo-product-test', 'process', text(language, 'Phản hồi thử nghiệm', 'Beta feedback'), 1010, 210),
    ...
    node('demo-product-metrics', 'database', text(language, 'Chỉ số', 'Metrics'), 300, 260),
  ]
...
function releaseChecklist(language: Language): DiagramSnapshot {
  const group = {
    ...node('demo-release-group', 'group', text(language, 'Cổng phát hành', 'Release gate'), 54, 36),
    style: { width: 780, height: 310 },
    zIndex: -1,
  }
  const freeze = { ...node('demo-release-freeze', 'process', text(language, 'Khóa tính năng', 'Feature freeze'), 48, 68), parentId: group.id, extent: 'parent' as const }
  const tests = { ...node('demo-release-tests', 'process', text(language, 'Chạy đủ cổng kiểm tra', 'Full gate'), 270, 68), parentId: group.id, extent: 'parent' as const }
  const ship = { ...node('demo-release-ship', 'decision', text(language, 'Có lỗi chặn?', 'Any blocker?'), 500, 54), parentId: group.id, extent: 'parent' as const }
  ...
  const rollback = { ...node('demo-release-rollback', 'note', text(language, 'Đường quay lui rõ ràng', 'Clear rollback path'), 506, 208), parentId: group.id, extent: 'parent' as const }
  const done = node('demo-release-done', 'start', text(language, 'Gắn thẻ v1.0.0', 'Tag v1.0.0'), 925, 150)
  ...
  return {
    ...
    edges: [
      ...
      { ...edge(ship.id, freeze.id, 'demo-release-e6', text(language, 'Có', 'Yes')), style: { stroke: '#be123c', strokeWidth: 2, strokeDasharray: '6 4' } },
    ],
  }
}
...
function architectureMap(language: Language): DiagramSnapshot {
  ...
  const pwa = node('demo-arch-pwa', 'process', text(language, 'Vỏ PWA', 'PWA shell'), 320, 110, 'client')
  const model = node('demo-arch-model', 'process', text(language, 'Mô hình tài liệu', 'Document model'), 560, 110, 'service')
  const worker = node('demo-arch-worker', 'process', text(language, 'Bộ xử lý ELK', 'ELK worker'), 560, 270, 'service')
  const exportNode = node('demo-arch-export', 'document', text(language, 'Xuất SVG/PNG/PDF', 'SVG/PNG/PDF export'), 800, 110, 'service')
  const storage = node('demo-arch-storage', 'database', 'IndexedDB', 1040, 110, 'data')
  const file = node('demo-arch-file', 'document', '.99diagrams.json', 1040, 270, 'data')
  ...
  return {
    ...
    edges: [
      ...
      edge(model.id, worker.id, 'demo-arch-e3', text(language, 'bố cục', 'layout')),
      edge(model.id, exportNode.id, 'demo-arch-e4'),
      edge(model.id, storage.id, 'demo-arch-e5', text(language, 'tự lưu', 'autosave')),
      edge(exportNode.id, file.id, 'demo-arch-e6', text(language, 'xuất', 'export')),
      edge(file.id, model.id, 'demo-arch-e7', text(language, 'nhập', 'import')),
    ],
  }
}

function swimlaneProcess(language: Language): DiagramSnapshot {
  const lane = node('demo-lane', 'swimlane', text(language, 'Phân loại lỗi', 'Bug triage'), 80, 70)
  lane.data.swimlane = {
    direction: 'vertical',
    lanes: [
      text(language, 'Người dùng', 'User'),
      text(language, 'Người bảo trì', 'Maintainer'),
      text(language, 'Người đóng góp', 'Contributor'),
      'CI',
    ],
  }
  ...
  const reproduce = { ...node('demo-lane-repro', 'process', text(language, 'Tái hiện lỗi', 'Reproduce'), 276, 80), parentId: lane.id, extent: 'parent' as const }
  ...
  const gate = { ...node('demo-lane-gate', 'process', text(language, 'Kiểm thử + biên dịch', 'Test + build'), 686, 224), parentId: lane.id, extent: 'parent' as const }
  const release = node('demo-lane-release', 'start', text(language, 'Hợp nhất', 'Merge'), 1060, 194)
  ...
  return {
    ...
    edges: [
      ...
      edge(assign.id, report.id, 'demo-lane-e4', text(language, 'Cần thêm thông tin', 'Need info')),
...
export const demoGallery: DemoDiagram[] = [
  {
    ...
    copy: {
      vi: { name: 'Luồng sản phẩm', description: 'Từ ý tưởng đến phản hồi thử nghiệm và phát hành.' },
  ...
  },
  {
    ...
    layers: defaultLayers,
    copy: {
      vi: { name: 'Danh sách kiểm tra phát hành', description: 'Khóa tính năng, đủ cổng kiểm tra, nhật ký thay đổi, SBOM và quay lui.' },
  ...
  },
  {
    ...
    layers: systemLayers,
    copy: {
      vi: { name: 'Bản đồ kiến trúc', description: 'PWA, mô hình tài liệu, bộ xử lý, lưu trữ và luồng xuất.' },
  ...
  },
  {
    ...
    layers: defaultLayers,
    copy: {
      vi: { name: 'Phân loại lỗi theo vai trò', description: 'Quy trình cộng đồng qua người dùng, người bảo trì, người đóng góp và CI.' },
