...
- Network smoke confirms the app only requests same-origin static assets on
  default load.
- Import guard smoke confirms valid `.99draw.json` files open, malformed JSON is
...
- Demo gallery smoke confirms every shipped demo opens, remains editable and
  exports valid `.99draw.json` on Chromium, Firefox and WebKit.
...
- Import a valid `.99draw.json` file.
...
```bash
docker build -t 99draw:release-candidate .
docker run --rm -p 8080:8080 99draw:release-candidate
```
...
99draw v1 is a static app. To roll back, redeploy the previous `dist/` artifact
or Docker image. Avoid changing the `.99draw.json` schema without a migration;
