# Kế hoạch hoàn thiện 99 Diagrams v1.0

## Mục tiêu đã khóa

99 Diagrams v1.0 là trình vẽ flowchart/workflow mã nguồn mở, local-first và tự host
...
`tests/e2e/import-guard.spec.ts` xác nhận import `.99diagrams.json` hợp lệ, JSON sai
...
dependencies. ADR 0002 đã chốt inbound-equals-outbound GPL-3.0-only, không yêu cầu CLA
...
- Hoàn thiện các thao tác node/edge: keyboard create/connect/delete/duplicate,
  context menu, reconnect, edge type, dashed/animated line và fit
  selection.
...
Mục tiêu: file `.99diagrams.json` đáng tin và output mở được ở nơi khác.
...
- Format canonical là `.99diagrams.json`; UI adapter không quyết định schema file.
